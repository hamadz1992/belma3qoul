...
function HomePage() {
  ...
  const newsItems = posts.length > 0 ? posts.map((post) => post.message?.trim()).filter(Boolean).slice(0, 4) : ['جديدنا أولًا بأول', 'تابعوا أحدث منشورات وعروض كل شيء بالمعقول', 'زورونا واكتشفوا الجديد']

  return (
    <div className="min-h-screen overflow-x-hidden bg-slate-50 pb-3">
      <section id="home" className="border-b border-rose-100 bg-white">
        <style>{'@keyframes belma3qoulTicker{from{transform:translateX(0)}to{transform:translateX(-50%)}}'}</style>
        <div className="mx-auto w-full max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          <div className="overflow-hidden rounded-2xl border border-rose-100 bg-gradient-to-r from-rose-600 via-rose-500 to-fuchsia-600 text-white shadow-lg shadow-rose-200/50">
            <div className="flex items-center gap-4 px-4 py-3">
              <span className="shrink-0 rounded-full bg-white/20 px-3 py-1 text-xs font-black">آخر الأخبار</span>
              <div className="min-w-0 flex-1 overflow-hidden" dir="rtl">
                <div className="flex w-max gap-12 whitespace-nowrap text-sm font-semibold" style={{ animation: 'belma3qoulTicker 28s linear infinite' }}>
                  {[...newsItems, ...newsItems].map((item, index) => <span key={`${item}-${index}`} className="inline-flex items-center gap-3"><span className="h-1.5 w-1.5 rounded-full bg-white" />{item}</span>)}
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="featured" className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><div className="grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <article className="rounded-[2rem] border border-white bg-white p-6 shadow-lg shadow-slate-300/20 sm:p-8"><SectionHeading eyebrow="المنشورات المميزة" title="" description="" />{featuredLoading ? <div className="mt-8 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-8 text-center text-slate-500">جارٍ تحميل المنشورات المميزة...</div> : featuredPosts.length === 0 ? <div className="mt-8 rounded-[1.5rem] border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-slate-500">لا توجد منشورات مميزة حاليًا.</div> : <div className="mt-8 grid gap-5 lg:grid-cols-3">{featuredPosts.map((post) => <article key={post.id} className="overflow-hidden rounded-[1.6rem] border border-slate-200 bg-slate-50 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">{post.videoUrl ? <video src={post.videoUrl} poster={post.imageUrl || undefined} controls playsInline preload="metadata" className="h-48 w-full bg-black object-cover" /> : post.imageUrl ? <img src={post.imageUrl} alt="" className="h-48 w-full object-cover" loading="lazy" /> : null}{post.videoUrl ? null : <div className="p-5"><div className="flex items-center justify-between gap-3"><span className="text-xs font-semibold tracking-[0.2em] text-rose-500 uppercase">Facebook</span><span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-rose-700 ring-1 ring-rose-100">مثبت</span></div><p className="mt-4 whitespace-pre-line text-sm leading-7 text-slate-600">{post.message || 'منشور بدون نص'}</p><div className="mt-5 flex items-center justify-between gap-3"><span className="text-xs text-slate-500">{formatDate(post.createdTime || '')}</span>{post.permalinkUrl ? <a href={post.permalinkUrl} target="_blank" rel="noreferrer" className="text-sm font-semibold text-rose-700 transition hover:text-rose-800">عرض المنشور ↗</a> : null}</div></div>}{post.videoUrl && post.permalinkUrl ? <div className="p-2 text-center"><a href={post.permalinkUrl} target="_blank" rel="noreferrer" className="text-xs font-semibold text-rose-700 hover:text-rose-800">عرض المنشور ↗</a></div> : null}</article>)}</div>}</article>
      </div></section>
