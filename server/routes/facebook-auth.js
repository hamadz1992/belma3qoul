...
export async function facebookCallback(req, res) {
  ...
  if (!APP_ID || !APP_SECRET) {
    redirectToFacebookPage(res, {
      facebook: 'error',
      message: 'Missing Facebook app configuration',
    })
    return
  }
  ...
  try {
    ...
    if (!pagesResponse.ok) {
      redirectToFacebookPage(res, {
        ...
        message:
          ...
          'تعذر جلب صفحات Facebook',
      })
      return
    }

    const configuredPageId = text(
      process.env.FACEBOOK_PAGE_ID
    )

    const pageData = configuredPageId
      ? pagesData.data.find(
          (page) => page.id === configuredPageId
        )
      : pagesData.data[0]
    ...
    if (!pageData) {
      redirectToFacebookPage(res, {
        facebook: 'error',
        message:
          'لم يتم العثور على صفحة Facebook المطلوبة',
      })
      return
    }
    ...
    await saveFacebookData({
      ...
      expiresAt: debugData?.data?.expires_at
        ? new Date(
            debugData.data.expires_at * 1000
          ).toISOString()
        : '',
    })
      ...
      JSON.stringify(
        {
          id: pageData.id || '',
          name: pageData.name || '',
          hasPageToken: Boolean(pageData.access_token),
        },
        null,
        2
      )
