...
export default function FeaturedPostsPage() {
  ...
  const loadFeaturedPosts = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('تعذر تحميل المنشورات المميزة')
      }
  ...
  const isFeatured = (postId: string) =>
    featuredPosts.some((post) => post.id === postId)
  ...
  const addFeaturedPost = async (post: FacebookPost) => {
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'فشل تثبيت المنشور')
      }
  ...
  const removeFeaturedPost = async (postId: string) => {
    try {
      ...
      const response = await fetch(
        `/api/admin/featured-posts?id=${encodeURIComponent(postId)}`,
        { method: 'DELETE' },
      )
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'فشل إلغاء تثبيت المنشور')
      }
        ...
        <div className="mb-8 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              اختر منشورات Facebook التي تريد إبرازها في الموقع.
            </p>
          </div>
          <button
            type="button"
            ...
            disabled={loading || featuredLoading}
            className="rounded-xl border bg-white px-5 py-3 text-sm font-medium shadow-sm hover:bg-slate-50 disabled:opacity-50"
          >
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {featuredPosts.map((post, index) => (
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="mb-3 text-xs font-semibold text-rose-600">
                      #{index + 1} منشور مميز
                    </div>
                    <p className="line-clamp-4 text-sm">{post.message}</p>
                    <button
                      type="button"
                      onClick={() => void removeFeaturedPost(post.id)}
                      disabled={busyId === post.id}
                      className="mt-5 w-full rounded-xl border border-red-200 py-3 text-red-600 transition hover:bg-red-50 disabled:opacity-50"
                    >
              ...
              {posts.map((post) => (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <button
                      type="button"
                      onClick={() => void addFeaturedPost(post)}
                      disabled={
                        busyId === post.id ||
                        isFeatured(post.id) ||
                        featuredPosts.length >= 3
                      }
                      className="mt-5 w-full rounded-xl bg-rose-600 py-3 text-white transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:bg-slate-300"
                    >
                      {busyId === post.id
                        ? 'جاري التثبيت...'
                        : isFeatured(post.id)
