        ...
        <section id="featured" className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="grid gap-5">
            ...
            <article className="order-2 overflow-hidden rounded-[1.5rem] border border-white bg-white shadow-[0_10px_28px_rgba(15,23,42,0.06)]">
              ...
              <div className="p-4 sm:p-5">
                {loading ? <div className="rounded-[1.2rem] border border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">جارٍ تحميل المنشورات...</div> : error ? <div className="rounded-[1.2rem] border border-rose-200 bg-rose-50 p-5 text-center text-sm text-rose-700">{error}</div> : posts.length === 0 ? <div className="rounded-[1.2rem] border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">لا توجد منشورات متاحة حاليًا.</div> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{posts.map((post) => <a key={post.id} href={post.permalinkUrl} target="_blank" rel="noreferrer" className="group block overflow-hidden rounded-[1.2rem] border border-slate-200 bg-white transition-all duration-300 hover:-translate-y-0.5 hover:border-rose-200 hover:shadow-[0_8px_20px_rgba(244,63,94,0.1)]">
