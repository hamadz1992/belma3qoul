      ...
      <div className="relative z-10">
        ...
        <section id="featured" className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="grid gap-5 xl:grid-cols-[330px_minmax(0,1fr)]">
            <article className="overflow-hidden rounded-[1.5rem] border border-white bg-white shadow-[0_10px_28px_rgba(15,23,42,0.06)]">
              <div className="border-b border-slate-100 bg-gradient-to-r from-rose-50 via-white to-pink-50 px-5 py-4 sm:px-6">
                <SectionHeading eyebrow="جديدنا" title="" description="" />
              </div>
              <div className="p-4 sm:p-5">
                {loading ? <div className="rounded-[1.2rem] border border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">جارٍ تحميل المنشورات...</div> : error ? <div className="rounded-[1.2rem] border border-rose-200 bg-rose-50 p-5 text-center text-sm text-rose-700">{error}</div> : posts.length === 0 ? <div className="rounded-[1.2rem] border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">لا توجد منشورات متاحة حاليًا.</div> : <div className="space-y-3">{posts.map((post) => <a key={post.id} href={post.permalinkUrl} target="_blank" rel="noreferrer" className="group block overflow-hidden rounded-[1.2rem] border border-slate-200 bg-white transition-all duration-300 hover:-translate-y-0.5 hover:border-rose-200 hover:shadow-[0_8px_20px_rgba(244,63,94,0.1)]">
                  <div className="relative h-32 overflow-hidden bg-slate-100">
                    ...
                    <div className="absolute right-2.5 top-2.5 flex items-center gap-1.5"><span className="rounded-full bg-white/95 px-2.5 py-1 text-[9px] font-bold text-rose-600 shadow-sm">Facebook</span>{post.videoUrl ? <span className="rounded-full bg-black/60 px-2 py-1 text-[9px] font-semibold text-white backdrop-blur">▶ فيديو</span> : null}</div>
                    <span className="absolute bottom-2.5 right-2.5 rounded-full bg-white/90 px-2.5 py-1 text-[9px] font-medium text-slate-700 shadow-sm backdrop-blur">{formatDate(post.createdTime)}</span>
                  </div>
                  <div className="p-3.5 text-right">
                    <p className="text-[12px] font-medium leading-5 text-slate-700" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{post.message || 'منشور جديد من صفحتنا'}</p>
                    <div className="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-2.5"><span className="text-[11px] font-semibold text-rose-600 transition group-hover:text-rose-700">عرض المنشور ↗</span><span className="text-[10px] text-slate-400">منشور جديد</span></div>
                  </div>
                </a>)}</div>}
              </div>
            </article>
            ...
            <article className="overflow-hidden rounded-[1.5rem] border border-white bg-white shadow-[0_10px_28px_rgba(15,23,42,0.06)]">
              <div className="border-b border-slate-100 bg-gradient-to-r from-rose-50 via-white to-amber-50 px-5 py-4 sm:px-6">
                <SectionHeading eyebrow="المنشورات المميزة" title="" description="" />
              </div>
              <div className="p-4 sm:p-5">
                {featuredLoading ? <div className="rounded-[1.2rem] border border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">جارٍ تحميل المنشورات المميزة...</div> : featuredPosts.length === 0 ? <div className="rounded-[1.2rem] border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">لا توجد منشورات مميزة حاليًا.</div> : <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{featuredPosts.map((post) => <article key={post.id} className="group overflow-hidden rounded-[1.25rem] border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-rose-200 hover:shadow-[0_10px_24px_rgba(244,63,94,0.1)]">
                  ...
                  <div className="p-3.5 text-right">
                    <p className="text-[12px] font-medium leading-5 text-slate-700" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{post.message || 'منشور مميز من صفحتنا'}</p>
                    <div className="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-2.5"><span className="text-[10px] text-slate-400">{formatDate(post.createdTime || '')}</span>{post.permalinkUrl ? <a href={post.permalinkUrl} target="_blank" rel="noreferrer" className="text-[11px] font-bold text-rose-600 transition hover:text-rose-700">عرض المنشور ↗</a> : null}</div>
                  </div>
                </article>)}</div>}
              </div>
            </article>
