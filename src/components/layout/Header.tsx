...
const socialPlatforms = [
  ...
  { key: 'telegram', label: 'Telegram', hint: 'قناتنا على Telegram', platform: 'telegram' },
  { key: 'youtube', label: 'YouTube', hint: 'شاهد فيديوهاتنا', platform: 'youtube' },
] as const
    ...
    fetch('/api/settings', { cache: 'no-store' })
      ...
      .then((data) => {
        if (mounted && data?.social) {
          setSocial((current) => ({ ...current, ...data.social }))
        }
      })
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-rose-100/80 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl items-center gap-4 px-4 py-4 sm:px-6 lg:px-8">
        <a
          href="#home"
          aria-label="Belma3qoul"
          className="shrink-0"
        >
          <BrandMark compact />
        </a>

        {/* روابط التواصل - نفس تصميم قسم تواصل معنا */}
        <div className="hidden flex-1 items-center justify-end gap-2 xl:flex">
          {socialPlatforms
            .filter((link) => Boolean(social[link.key].trim()))
            .map((link) => {
              const accentClass =
                platformAccent[link.platform] || 'bg-slate-900 text-white'
              ...
              return (
                <a
                  ...
                  rel="noreferrer"
                  title={link.label}
                  className="flex min-w-[145px] items-center gap-3 rounded-[1.3rem] border border-slate-100 bg-slate-50 px-3 py-2 transition hover:-translate-y-0.5 hover:border-rose-200 hover:bg-rose-50"
                >
                  <span
                    className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold shadow-sm ${accentClass}`}
                  >
        ...
        <div className="ml-auto flex items-center gap-3 xl:hidden">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 rounded-[1.2rem] border border-slate-100 bg-slate-50 px-3 py-2 transition hover:border-rose-200 hover:bg-rose-50"
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#25D366] text-xs font-bold text-white shadow-sm">
              W
            </span>
