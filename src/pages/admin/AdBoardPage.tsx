        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          <section className="rounded-3xl bg-white p-6 shadow">
            ...
            <select value={ads.type} onChange={(e) => setAds({ ...ads, type: e.target.value as AdType })} className="mb-5 w-full rounded-xl border p-3">
              <option value="post">📝 منشور إعلاني</option><option value="image">🖼️ صورة إعلان</option><option value="banner">🖥️ بانر ثابت</option><option value="slider">🎞️ بانر متحرك</option><option value="video">▶️ إعلان فيديو</option>
            </select>

            {(ads.type === 'post' || ads.type === 'image' || ads.type === 'banner') && <>
              <label className="mb-2 block font-semibold">صورة الإعلان</label>
              <div className="mb-5 flex flex-wrap items-center gap-3">
                <label className="cursor-pointer rounded-xl bg-slate-900 px-5 py-3 font-semibold text-white hover:bg-slate-800">📁 اختيار صورة من الجهاز<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => void handleImage(e)} className="hidden" /></label>
                {ads.imageUrl && <button type="button" onClick={() => setAds({ ...ads, imageUrl: '' })} className="rounded-xl border border-red-200 px-4 py-3 text-red-600 hover:bg-red-50">حذف الصورة</button>}
              </div>
